    ...
    383: function (t, e) {
        !(function () {
            ...
            var t = [],
                e = [],
                r = [],
                a = moment().subtract(6, "days").format("YYYY-MM-DD"),
                n = moment().format("YYYY-MM-DD");
            moment.range(a, n).by("days", function (a) {
                t.push({ y: Math.floor(300 * Math.random()) + 30, x: a.toDate() }),
                    e.push({ y: Math.floor(300 * Math.random()) + 10, x: a.toDate() }),
                    a.startOf("day").isSame(moment().startOf("day")) ? r.push(settings.colors.accent[500]) : r.push(settings.colors.primary[500]);
            });
            !(function (r) {
                ...
                (function (t) {
                    ...
                    var a = [],
                        n = [],
                        o = moment().subtract(7, "days").format("YYYY-MM-DD"),
                        i = moment().format("YYYY-MM-DD"),
                        s = moment.range(o, i);
                    s.by("days", function (t) {
                        var e = Math.floor(300 * Math.random()) + 10;
                        a.push({ y: e, x: t.toDate() }), n.push({ y: Math.round(0.5 * e), x: t.toDate() });
                    });
                    a = {
                        datasets: [
                            { label: "Conversion", data: n },
                            { label: "Views", data: a },
                        ],
                    };
                    Charts.create(t, e, r, a);
                })("#viewsChart");
        })();
    },
});